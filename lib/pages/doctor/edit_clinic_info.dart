...
class EditClinicInfo extends StatefulWidget {
  const EditClinicInfo({Key key}) : super(key: key);

  @override
  _EditClinicInfoState createState() => _EditClinicInfoState();
}

class _EditClinicInfoState extends State<EditClinicInfo> {
  ...
  void showPlacePicker() async {
    double initalLang = selectedLng == null || selectedLng == "null"
        ? null
        : double.parse(selectedLng);
    double initalLat = selectedLat == null || selectedLat == 'null'
        ? null
    ...
    AppUtils.showToast(
        ...
        backgroundColor: const Color(0xff3CDF9E));
  ...
  Future<void> getImage(
    ...
    if (source == ImageSource.camera) {
      ...
      if (pickedFiles != null) {
        if (mounted) {
          setState(() {
            galleryList.addAll(pickedFiles);
          });
        }
      }
    }
  ...
  void validateAndSubmit() {
    ...
    if (_formKey.currentState.validate()) {
      ...
      _authController.updateInfo(body: formData, isFirstTime: false);
    }
  }

  void initUserData() {
    _priceController.text =
        _authController.activeUserData.value.price.toString();
    infoEnController.text = _authController.activeUserData.value.clinicInfoEn;
    infoArController.text = _authController.activeUserData.value.clinicInfoAr;
    addressArController.text = _authController.activeUserData.value.addressAr;
    addressEnController.text = _authController.activeUserData.value.addressEn;
    servicesArItemList = _authController.activeUserData.value.clinicServiceAr;
    servicesEnItemList = _authController.activeUserData.value.clinicServiceEn;
    if (_authController.activeUserData.value.city != null) {
      _selectedGovernorates = _authController.activeUserData.value.city.id;
    }
    _generalDataController.getDistrictList(_selectedGovernorates);
    if (_authController.activeUserData.value.district != null) {
      _selectedArea = _authController.activeUserData.value.district.id;
    }
    _acceptEmergency =
        _authController.activeUserData.value.isEmergenc == 1 ? true : false;

    if (_authController.activeUserData.value.lat != null &&
        _authController.activeUserData.value.long != null) {
      selectedLng = _authController.activeUserData.value.long;
      selectedLat = _authController.activeUserData.value.lat;
    }
  }
}
